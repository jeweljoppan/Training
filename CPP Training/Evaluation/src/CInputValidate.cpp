...
#include "../include/CInputValidate.h"
...
// Parameters  :
//  <param 1>  - < char pEmail >   - Input array containing Email.
// Returns     : int
// Description : Checks and validates whether user inputs is a email
//               in valid format
// ------------------------------------------------------------------
bool CInputValidate::ValidateEmail( char pEmail[] )
{
    string cEmail(pEmail);
    const regex pattern
    ("(\\w+)(\\.|_)?(\\w*)@(\\w+)(\\.(\\w+))+");

    return regex_match(cEmail,pattern);
